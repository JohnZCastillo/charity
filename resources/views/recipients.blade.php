...
@section('body')
                    ...
                    <tr>
                        ...
                        <td>
                            @if($recipient->status == 'enabled')
                                <div class="badge badge-success">{{$recipient->status}}</div>
                            @else
                                <div class="badge disabled">{{$recipient->status}}</div>
                            @endif
                        </td>
                        <td>
                            <div class="flex gap-2">
                                <a type="button" href="/item/edit/{{$recipient->id}}">
                                    <i class='bx bx-sm bx-pencil'></i>
                                </a>
                                <form method="POST" action="/item/{{$recipient->id}}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit">
    ...
    <dialog id="donorModal" class="modal">
        <div class="modal-box">
            ...
            <form enctype="multipart/form-data" class="w-full" method="POST" action="/recipient">
                @csrf
